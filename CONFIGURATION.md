...
- `LLM_EXTRA_HEADERS_JSON` — optional JSON object of additional HTTP headers
  to send with LLM provider requests. Set non-sensitive values via
  `*.env.llmExtraHeaders`; for sensitive header values, set
  `LLM_EXTRA_HEADERS_JSON` through `*.extraEnv` and a Kubernetes Secret.
...
frontend:
  env:
    ...
    # Optional: extra non-sensitive HTTP headers for provider requests
    # llmExtraHeaders:
