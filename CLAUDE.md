...
`frontend.subPath.enabled` lets self-hosters serve Laminar under the fixed `/lmnr` sub-path of an existing domain (e.g. `app.company.com/lmnr`) instead of a dedicated hostname (LAM-1749, for OpenHands' bundled deployment). The prefix is **baked into the frontend image at build time** (Next.js `basePath` inlines it into the standalone bundle's asset URLs — it can't be flipped purely at runtime), so the chart ships a separate `frontend-ee-basepath` image built with `/lmnr`. The bool drives BOTH the image selection (`lmnr.frontend.imageName` helper forces `frontend-ee-basepath` when enabled, overriding `images.frontend.name`) AND the ingress path (`lmnr.frontend.subPath` helper returns `/lmnr`), so they can't disagree — this replaced the earlier free-form `frontend.basePath` string, which let operators point the ingress at a prefix the image wasn't built with. The prefix is intentionally NOT configurable: `/lmnr` is the only published image build. `frontend.ingress.path` defaults to the helper's `/lmnr` (or `/` when disabled); override only when the upstream proxy rewrites the prefix. Operators must also set `nextPublicUrl` to include the prefix. Default is `subPath.enabled: false` (root-served, regular `frontend-ee` image, byte-identical to before). The OSS `frontend-ee-basepath` image target is built in `lmnr-private` CI, not here.
